...
from data.mongodb import encontra_ensaio
from main import ensaio


def start():

    global ensaio_iniciar, AcoesEletroima, Poses, Detalhes

    ensaio_iniciar = encontra_ensaio(ensaio)

    print(ensaio_iniciar["_id"])
    ...
    class Poses(Enum):
        ...
        X_BANDEJA_A = ensaio_iniciar["x1"]
        Y_BANDEJA_A = ensaio_iniciar["y1"]
        DESLOCAMENTO_X_BANDEJA_A = ensaio_iniciar["x1_e"]
        DESLOCAMENTO_Y_BANDEJA_A = ensaio_iniciar["y1_e"]

        X_BANDEJA_B = ensaio_iniciar["x2"]
        Y_BANDEJA_B = ensaio_iniciar["y2"]
        DESLOCAMENTO_X_BANDEJA_B = ensaio_iniciar["x2_e"]
        DESLOCAMENTO_Y_BANDEJA_B = ensaio_iniciar["y2_e"]

        X_BANDEJA_C = ensaio_iniciar["x3"]
        Y_BANDEJA_C = ensaio_iniciar["x3"]
        DESLOCAMENTO_X_BANDEJA_C = ensaio_iniciar["x3_e"]
        DESLOCAMENTO_Y_BANDEJA_C = ensaio_iniciar["x3_e"]

    # Configurações do ensaio
    class Detalhes(Enum):
        NUMERO_VARRERUDAS_AMOSTRA = ensaio_iniciar["vrr"]
        NUMERO_CICLOS_ENSAIO = ensaio_iniciar["cic"]
